...
class LandmarkAligner:
    ...
    def align_to_template(
        ...
        def direct_aspect_fallback() -> Tuple[np.ndarray, np.ndarray, np.ndarray, CoordinateTransform, bool]:
            M_fw = np.array([
            ...
            return canonical_img, M_fw, M_inv, coord_tx, False

        if image is None or image.size == 0:
            return direct_aspect_fallback()

        # Create target canonical reference canvas
            ...
            if len(matches) >= 6:
                ...
                if M_forward is not None and M_forward.shape == (3, 3):
                    try:
                        ...
                        # SAFEGUARD: Determinant check to prevent perspective streaking singularities
                        if not np.isnan(det) and not np.isinf(det) and 0.01 <= abs(det) <= 100.0:
